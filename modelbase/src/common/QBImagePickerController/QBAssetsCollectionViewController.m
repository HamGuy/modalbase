...
- (void)setAllowsMultipleSelection:(BOOL)allowsMultipleSelection
{
    ...
    if (allowsMultipleSelection) {
        ...
        [self addRightBarButtonWithName:@"完成" withAction:@selector(done:)];
    } else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.title = [NSString stringWithFormat:@"已选中 %d 张照片",self.imagePickerController.selectedAssetURLs.count];
}
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger count = self.imagePickerController.selectedAssetURLs.count;
    if (count==0) {
        self.title = [self.assetsGroup valueForProperty:ALAssetsGroupPropertyName];
    }else{
        self.title = [NSString stringWithFormat:@"已选中 %d 张照片",count];
    }
}
@end
