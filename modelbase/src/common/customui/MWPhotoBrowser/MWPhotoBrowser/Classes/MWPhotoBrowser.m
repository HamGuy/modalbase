...
- (void)viewDidLoad {
    ...
    if (IS_IOS6()) {
        //根据需求自定
        _toolBarSix = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"BgTitleBarDownSide"]];
        _toolBarSix.frame = [self frameForToolbarAtOrientation:self.interfaceOrientation];
        _toolBarSix.userInteractionEnabled = YES;
    }else{
        _toolbar = [[UIToolbar alloc] initWithFrame:[self frameForToolbarAtOrientation:self.interfaceOrientation]];
        _toolbar.backgroundColor = RGBACOLOR(0, 0, 0,179);
...
//        _nextButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:[NSString stringWithFormat:arrowPathFormat, @"Right"]] style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
        UIButton *preBtn = [self buttonWithImage:@"BtnPreVious" hilightedImage:@"BtnPreViousPress" selector:@selector(gotoPreviousPage)];
        preBtn.frame = CGRectMake(2, 6, 60, 44);
        _previousButton = [[UIBarButtonItem alloc] initWithCustomView:preBtn];
        
//        UIButton *likeBtn = [self buttonWithImage:@"BtnLike" hilightedImage:@"BtnLikePress" selector:@selector(likeThis:)];
//        likeBtn.frame = CGRectMake(124, 6, 72, 44);
//        _likeButton = [[UIBarButtonItem alloc] initWithCustomView:likeBtn];
    
        UIButton *nextBtn = [self buttonWithImage:@"BtnNext" hilightedImage:@"BtnNextPress" selector:@selector(gotoNextPage)];
        nextBtn.frame = CGRectMake(257, 6, 60, 44);
        _nextButton = [[UIBarButtonItem alloc] initWithCustomView:nextBtn];
//    }
//    if (self.displayActionButton) {
        UIButton *actionBtn = [self buttonWithImage:@"IconShare" hilightedImage:@"IconSharePress" selector:@selector(actionButtonPressed:)];
        actionBtn.frame = CGRectMake(0, 0, 30, 30);
        actionBtn.adjustsImageWhenHighlighted = NO;
        _actionButton = [[UIBarButtonItem alloc] initWithCustomView:actionBtn];
//    }
...
-(void)updateLikeButtonStatus:(BOOL)couldLike{
    UIButton *btn = nil;
    if (_likeButton == nil) {
        btn = [self buttonWithImage:@"BtnLike" hilightedImage:@"BtnLikePress" selector:@selector(likeThis:)];
        btn.frame = CGRectMake(124, 6, 72, 44);
        _likeButton = [[UIBarButtonItem alloc] initWithCustomView:btn];
    }else{
        btn = (UIButton *)[_likeButton customView];
    }
    //已经赞过
    if (!couldLike) {
        [btn setImage:[UIImage imageNamed:@"BtnUnlike"] forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:@"BtnUnlikPress"] forState:UIControlStateHighlighted];
    }else{
        [btn setImage:[UIImage imageNamed:@"BtnLike"] forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:@"BtnLikePress"] forState:UIControlStateHighlighted];
    }
...
//        previousViewController.navigationItem.backBarButtonItem = newBackButton;
    ...
    if (_previousButton && _nextButton && numberOfPhotos > 1) {
        if (IS_IOS6()) {
            [_toolBarSix addSubview:_previousButton.customView];
            [_toolBarSix addSubview:_likeButton.customView];
            [_toolBarSix addSubview:_nextButton.customView];
...
//    }
    if (hideToolbar) {
        ...
        [self.view addSubview:IS_IOS6()? _toolBarSix : _toolbar];
    }
...
-(UIView *)fadeView{
    if (_fadeView == nil) {
        ...
        _fadeView.frame = CGRectMake(0, 0, 320, 20);
...
- (void)layoutVisiblePages {
    ...
    if (IS_IOS6()) {
        _toolBarSix.frame = [self frameForToolbarAtOrientation:self.interfaceOrientation];
        _fadeView.alpha = 0;
        [[UIApplication sharedApplication].keyWindow addSubview:self.fadeView];
    }else{
...
-(UIButton *)buttonWithImage:(NSString *)image hilightedImage:(NSString *)hilightedImage selector:(SEL)selector{
...
-(IBAction)likeThis:(id)sender{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickLikeButton:)]) {
        [self.delegate didClickLikeButton:self];
    }
}
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_7_0
            if (SYSTEM_VERSION_LESS_THAN(@"7"))
                fullScreen = self.wantsFullScreenLayout;
#endif
            if (fullScreen) {
...
//                if (!IS_IOS6())
                ...
                navBarFrame.origin.y = IS_IOS6() ? 0 : statusBarHeight;
    ...
    if (slideAndFade && [self areControlsHidden] && !hidden && animated) {
        ...
        // Toolbar
        if (IS_IOS6()) {
            _toolBarSix.frame = CGRectOffset([self frameForToolbarAtOrientation:self.interfaceOrientation], 0, animatonOffset);
        }else{
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        if (slideAndFade) {
            ...
            if (hidden){
                if (IS_IOS6()) {
                    _toolBarSix.frame = CGRectOffset(_toolbar.frame, 0, animatonOffset);
                }else{
...
- (BOOL)areControlsHidden { return IS_IOS6() ? (_toolBarSix.alpha == 0):(_toolbar.alpha == 0); }
